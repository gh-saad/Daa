<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" dir="{{ company_setting('site_rtl',$company_id) == 'on'?'rtl':''}}">
<meta name="csrf-token" id="csrf-token" content="{{ csrf_token() }}">
<head>
<meta charset="UTF-8">
<meta content="width=device-width, initial-scale=1, maximum-scale=1, shrink-to-fit=no" name="viewport">
<title>@yield('page-title') | {{ !empty(company_setting('title_text',$company_id)) ? company_setting('title_text',$company_id) : (!empty(admin_setting('title_text')) ? admin_setting('title_text') :'DAA ERP') }}</title>
...
<link rel="icon" href="{{ get_file(favicon())}}{{'?'.time()}}" type="image/x-icon" />
 ...
 @if (company_setting('site_rtl',$company_id,$workspace_id) == 'on')
...
<script src='https://unpkg.com/nprogress@0.2.0/nprogress.js'></script>
    ...
    <body class="{{ !empty(company_setting('color',$company_id,$workspace_id))?company_setting('color',$company_id,$workspace_id):'theme-3' }}">
        <div class="container">
