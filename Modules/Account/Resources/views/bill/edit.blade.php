    ...
    <div class="row">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <div class="form-icon-user">
                                            {{ Form::date('bill_date', \Carbon\Carbon::parse($bill->bill_date)->format('Y-m-d'), ['class' => 'form-control', 'required' => 'required', 'placeholder' => 'Select Issue Date']) }}
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <div class="form-icon-user">
                                            {{ Form::date('due_date', \Carbon\Carbon::parse($bill->due_date)->format('Y-m-d'), ['class' => 'form-control', 'required' => 'required', 'placeholder' => 'Select Due Date']) }}
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                ...
                                <div class="col-md-6">
                                    ...
                                    @if (module_is_active('Account'))
                                        ...
                                        {{ Form::select('category_id', $category,$bill->category_id, array('class' => 'form-control','required'=>'required')) }}
        ...
        <div class="col-12">
            <div class="modal-footer">
                    ...
                    <input type="submit" value="{{ __('Update') }}" class="btn btn-primary mx-3">
                </div>
            </div>
        </div>
        {{ Form::close() }}
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function fetchVendorDetails(vendorId) {
                const url = vendorSelect.attr('data-url').replace(':id', vendorId);
            ...
            // Trigger the fetch on change
            vendorSelect.on('change', function () {
                const vendorId = this.value;
                fetchVendorDetails(vendorId);
            });

            // Fetch vendor details on page load for the preselected vendor
            const selectedVendorId = vendorSelect.val();
            if (selectedVendorId) {
    ...
    <script>
        $(document).ready(function() {
            var productsSelectedArray = @json($company_products_selected_array);

            // Convert productsSelectedArray to an actual array if it's an object
            if (typeof productsSelectedArray === 'object') {
                productsSelectedArray = Object.values(productsSelectedArray);
            }

            // Function to load pre-selected items into the table and hidden inputs
            function loadPreSelectedItems() {
                let hasMissingItems = false; // Track if any items are skipped

                productsSelectedArray.forEach(function(item) {

                    const itemId = item.product_id;
                    const itemProductType = item.type;
                    const itemName = item.name;
                    const itemPrice = parseFloat(item.price);
                    const itemDiscount = parseFloat(item.discount);
                    const itemAccountName = item.account_name;
                    const itemAccountID = item.account_id;
                    const itemDesc = item.desc;

                    // Check if any of the required fields are null, and skip if so
                    if (itemId === null || itemProductType === null || itemPrice === null || itemAccountID === null) {
                    ...
                    $('#items').append(`
                        <tr data-item-id="${itemId}">
                            ...
                            <td>${itemPrice.toFixed(2)} {{ company_setting('defult_currancy') }}</td>
                            <td>${itemDiscount.toFixed(2)} {{ company_setting('defult_currancy') }}</td>
                    ...
                    $('#item-inputs').append(`
                        <div class="item-input-group" data-item-id="${itemId}">
                            ...
                            <input type="hidden" name="item_accounts[]" value="${itemAccountID}">
                            <input type="hidden" name="item_types[]" value="${itemProductType}">
                            <input type="hidden" name="item_prices[]" value="${itemPrice.toFixed(2)}">
                            <input type="hidden" name="item_discounts[]" value="${itemDiscount.toFixed(2)}">
                ...
                // If any items were skipped due to missing data, show a toaster message
                if (hasMissingItems) {
                    toastr.error('Some items were skipped due to missing data, Please double check this bill draft before submitting it.');
                }

                // Recalculate totals after loading the pre-selected items
                recalculateTotals();
            }

            // Load pre-selected items on page load
